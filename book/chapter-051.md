...
CrewAI-style roles: agents, tasks, sequential crew kickoff.

Part VI compares **frameworks** after you built agents from first principles (Parts IV–V). This chapter teaches the **mental model and control surface** of CrewAI via an offline adapter in `code/chapter-051/crewx/` so you can map it onto your platform without treating the framework as architecture.

## Learning Objectives

- Explain core CrewAI primitives
- Map them to harness/tools/memory/graph ports from earlier chapters
- Run the offline adapter tests
- Know when adopting CrewAI helps vs hides critical control flow
...
```text
code/chapter-051/crewx/runtime.py
```
...
Offline **CrewAI** adapter demonstrating the framework’s primary runtime loop.
