...
OpenAI Agents-style runner: agent, tools, handoffs, session trace.

Part VI compares **frameworks** after you built agents from first principles (Parts IV–V). This chapter teaches the **mental model and control surface** of OpenAI Agents SDK via an offline adapter in `code/chapter-049/oaiagents/` so you can map it onto your platform without treating the framework as architecture.

## Learning Objectives

- Explain core OpenAI Agents SDK primitives
- Map them to harness/tools/memory/graph ports from earlier chapters
- Run the offline adapter tests
- Know when adopting OpenAI Agents SDK helps vs hides critical control flow
...
```text
code/chapter-049/oaiagents/runtime.py
```
...
Offline **OpenAI Agents SDK** adapter demonstrating the framework’s primary runtime loop.
