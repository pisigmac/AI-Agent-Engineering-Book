...
Think-Act-Observe engine with retry and terminate conditions.
...
- Prior Part IV chapters through 28

## Motivation

Agents fail in production when autonomy is unbounded, tools are ungoverned, or state is implicit. This chapter makes **execution loops** an explicit, testable subsystem.

## First Principles

### 1. TAO loop structure
...
Run the package CLI/tests under `code/chapter-029/`. Key entrypoints live in the `execloop` package.
...
| Diagrams | `diagrams/mermaid|png/chapter-029/` |
...
```bash
cd code/chapter-029 && pytest -q && python3 main.py --help || python3 main.py
```

## Chapter Summary

Think-Act-Observe engine with retry and terminate conditions. Deliverable: **Loop history + reason**.

## What's Next

**Chapter 30** continues Part IV.