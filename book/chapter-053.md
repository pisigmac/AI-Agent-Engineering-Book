...
PydanticAI-style typed agent: deps, tools, structured result model.

Part VI compares **frameworks** after you built agents from first principles (Parts IV–V). This chapter teaches the **mental model and control surface** of PydanticAI via an offline adapter in `code/chapter-053/pydanticaix/` so you can map it onto your platform without treating the framework as architecture.

## Learning Objectives

- Explain core PydanticAI primitives
- Map them to harness/tools/memory/graph ports from earlier chapters
- Run the offline adapter tests
- Know when adopting PydanticAI helps vs hides critical control flow
...
```text
code/chapter-053/pydanticaix/runtime.py
```
...
Offline **PydanticAI** adapter demonstrating the framework’s primary runtime loop.
