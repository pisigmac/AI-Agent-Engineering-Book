...
AutoGen-style multi-agent chat: user proxy + assistant + termination.

Part VI compares **frameworks** after you built agents from first principles (Parts IV–V). This chapter teaches the **mental model and control surface** of AutoGen via an offline adapter in `code/chapter-052/autogenx/` so you can map it onto your platform without treating the framework as architecture.

## Learning Objectives

- Explain core AutoGen primitives
- Map them to harness/tools/memory/graph ports from earlier chapters
- Run the offline adapter tests
- Know when adopting AutoGen helps vs hides critical control flow
...
```text
code/chapter-052/autogenx/runtime.py
```
...
Offline **AutoGen** adapter demonstrating the framework’s primary runtime loop.
