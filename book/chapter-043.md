...
Approval gates, interrupts, and escalation for high-risk agent actions.
...
- Prior Part V chapters through 42

## Motivation

Demo agents break under real traffic: no budgets, no approvals, no metrics, no security boundary, no scale plan. This chapter makes **Human-in-the-Loop** an operable subsystem.

## First Principles

### 1. Risk-tiered approvals

### 2. Interrupts are not errors

### 3. Escalation paths must be explicit
...
See `code/chapter-043/` for the `hitl` package, CLI, and tests.
