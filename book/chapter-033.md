...
Working, episodic, and semantic memory with retrieval.
...
- Prior Part IV chapters through 32

## Motivation

Agents fail in production when autonomy is unbounded, tools are ungoverned, or state is implicit. This chapter makes **memory systems** an explicit, testable subsystem.

## First Principles

### 1. Memory kinds and lifetimes

### 2. Working buffer capacity
...
Run the package CLI/tests under `code/chapter-033/`. Key entrypoints live in the `memsys` package.
...
| Diagrams | `diagrams/mermaid|png/chapter-033/` |
...
```bash
cd code/chapter-033 && pytest -q && python3 main.py --help || python3 main.py
```

## Chapter Summary

Working, episodic, and semantic memory with retrieval. Deliverable: **Layered context bundle**.

## What's Next

**Chapter 34** continues Part IV.