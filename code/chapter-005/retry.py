"""HTTP retry policy for the platform client."""
...
class RetryPolicy:
    ...
    def allow(
        ...
        """Return True if another attempt should be made after this failure.

        ``attempt`` is the 1-based attempt that just finished/failed.
        """
        if attempt >= self.max_attempts:
            return False
        if status is None:
            # network / timeout — only if safe
        ...
        if status in TRANSIENT:
            if method.upper() == "POST" and not idempotent:
                return False
            return idempotent or method.upper() in {"GET", "HEAD", "OPTIONS", "PUT", "DELETE"}
        return False
